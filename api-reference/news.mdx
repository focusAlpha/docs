...
The news layer answers two different questions against two different keys: *what has been happening to a company* (the per-company routes) and *what has happened on a macro theme* — rates, trade, geopolitics — most of which names no company and is therefore unreachable by ticker. Sources span press and wire coverage (roughly 160 feeds across two dozen regions), earnings-call transcripts, SEC 8-K filings, Asian exchange filings, company IR pages, and a semiconductor supply-chain lane; `channel` on every row says which.
