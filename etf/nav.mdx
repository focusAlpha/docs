...
`market_price` is the closing market price of the listing, and `premium_discount` is the published gap between market price and NAV. `split_factor` carries the adjudicated split applied on that date.
